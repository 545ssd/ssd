/*
 * NAND Peripheral between the Fast Simplex Link (FSL) of the Microblaze
 * and a buffer, NAND controller and ECC module*
 *
 * *to be completed
 *
 * Author: Timothy Tan
...
wire [0:31] nAddr;
wire [0:2] nCmd;
wire nData_Loaded, nCmd_Loaded;
wire n_re_l, n_we_l, n_ale, n_cle, n_ce1_l, n_ce2_l;
wire [0:7] n_io;
...
// nand model
nand_model mt29( .Dq_Io(n_io), .Cle(n_cle), .Ale(n_ale), .Ce_n(n_ce1_l), .Ce2_n(n_ce2_l),
	.Clk_We_n(n_we_l), .Wr_Re_n(n_re_l), .Wp_n(1'b1), .Rb_n(n_rb1_l), .Rb2_n(n_rb2_l));


initial begin
//	$display(" FSL_S_Clk, FSL_S_Read, FSL_M_Clk, FSL_M_Write, FSL_M_Control, FSL_M_Data, FSL_Clk, FSL_S_Data, FSL_S_Control, FSL_S_Exists, FSL_M_Full, state, control, count, raddr, nDone, nReset, nAddr, nCmd, nCmd_Loaded, nData_Loaded" );
//	$monitor($time,,,"%b%b%b%b%b %x %b %x %b%b%b %x %x %x %x %b%b %x %x %b%b", FSL_S_Clk, FSL_S_Read, FSL_M_Clk, FSL_M_Write, FSL_M_Control, FSL_M_Data, FSL_Clk, FSL_S_Data, FSL_S_Control, FSL_S_Exists, FSL_M_Full, state, control, count, raddr, nDone, nReset, nAddr, nCmd, nCmd_Loaded, nData_Loaded);
...
//	program_page(32'h80002000);
//	read_page(32'h80002000);
//	erase_block(32'h80002000);
//	read_erased_page(32'h80002000);
//
//	program_page(32'h00000000);
//	program_page(32'h00002000);
//	program_page(32'h00004000);
//	program_page(32'h00006000);
//	program_page(32'h00008000);
//	program_page(32'h0000a000);
//	program_page(32'h0000c000);
//	program_page(32'h0000e000);
//	read_page(32'h00008000);
//	read_page(32'h0000a000);
//	erase_block(32'h0);
//	read_erased_page(32'h00008000);
//	program_page(32'h00012000);
//	read_erased_page(32'h0000a000);
//	read_page(32'h00012000);
...
always begin
	#10 FSL_M_Full = 1;//~FSL_M_Full;
	#30 FSL_M_Full = 0;//~FSL_M_Full;
end
...
begin
	...
	FSL_S_Exists = 1;
	FSL_S_Control = 0;
	FSL_S_Data = 32'h00100002;
	@(posedge FSL_Clk);
	FSL_S_Data = addr;
	@(posedge FSL_Clk);
	FSL_S_Data = 32'hdeadbee0;
	@(posedge FSL_Clk);
	FSL_S_Data = 32'hdeadbee1;
	@(posedge FSL_Clk);
	FSL_S_Data = 32'hdeadbee2;
	@(posedge FSL_Clk);
	FSL_S_Data = 32'hdeadbee3;
	@(posedge FSL_Clk);
	FSL_S_Exists <= 0;
	FSL_S_Data <= 0;
	#400000;
...
begin
	...
	while(~FSL_M_Write)
		#10;
	if (FSL_M_Data != 32'hdeadbee0)
		$display("expected %x but got %x", 32'hdeadbeef, FSL_M_Data);
	#10
	while(~FSL_M_Write)
		#10;
	if (FSL_M_Data != 32'hdeadbee1)
		$display("expected %x but got %x", 32'hdeadbeef, FSL_M_Data);
	#10
	while(~FSL_M_Write)
		#10;
	if (FSL_M_Data != 32'hdeadbee2)
		$display("expected %x but got %x", 32'hdeadbeef, FSL_M_Data);
	#10
	while(~FSL_M_Write)
		#10;
	if (FSL_M_Data != 32'hdeadbee3)
		$display("expected %x but got %x", 32'hdeadbeef, FSL_M_Data);
	$display("last word received = 0x%x", FSL_M_Data);
#20000;
end
endtask
...
begin
	...
	begin
		...
		while(~FSL_M_Write)
			#10;
		if (FSL_M_Data != (addr | i))
			$display("expected %x but got %x", addr | i, FSL_M_Data);
	end
	$display("last word received = 0x%x", FSL_M_Data);
#200;
end
...
begin
	...
	begin
		...
		while(~FSL_M_Write)
			#10;
		if (FSL_M_Data != 32'hffffffff)
			$display("expected %x but got %x", 32'hffffffff, FSL_M_Data);
	end
	$display("last word received = 0x%x", FSL_M_Data);
#200;
end
...
module nand_fsl_02 
	(
		...
		nData_Loaded,
		nCmd_Loaded,
		n_re_l, n_we_l, n_ale, n_cle, n_ce1_l, n_ce2_l, n_io_dir,
		n_io,
		n_rb1_l, n_rb2_l
...
output n_re_l, n_we_l, n_ale, n_cle, n_ce1_l, n_ce2_l, n_io_dir;
inout [0:7] n_io;
...
// MODULE nand_fsl_02 to implement your coprocessor
	...
	localparam In_Control = 6'h2;
	localparam Out_Control = 6'h3;
	localparam Program_Page = 6'h4;
	localparam Read_Data = 6'h5;
	localparam Block_Erase = 6'h6;
...
`endif
	...
	nand_controller_top c1( .DONE(nDone), .DATA_DONE(nData_Done), .DATA_READY(nData_Ready), .RB(nRB),
		.status(nStatus), .DATA_OUT(nData_Out), .DATA_IN(nData_In), .ADDR(nAddr), .CMD(nCmd),
	...
	/* tristate buffer */
	assign n_io = n_io_dir ? n_o : 8'hZZ;
	assign n_i = n_io_dir ? 8'h0 : n_io;
...
`ifdef SIMULATION
	...
	always@(posedge FSL_S_Read)
		$display("(%t) 0x%x ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ FSL_S_Read went high!", $time, FSL_S_Read);
