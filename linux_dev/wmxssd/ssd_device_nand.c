...
wmxssd_pcie_read_with_verify(char *buf, int size, unsigned int *lba, char *check)
{
   ...
   {
      if(check[j] != buf[j]){
         printk("WMXSSD-READ-TEST: exp:0x%x act:0x%x --------FAIL\n", check[j], buf[j]);
         return 1;
      }
   }
...
void test_pcie_ramdisk(void)
{
   ...
   unsigned char * ramdisk_writeaddr;
...
//       i = 5;
//       wmxssd_pcie_write(knownvalue_buffer, i, ramdisk_writeaddr);
//       ramdisk_writeaddr += i;
//       //16 byte
//       i = 16;
//       wmxssd_pcie_write(knownvalue_buffer, i, ramdisk_writeaddr);
...
//       i = 5;
//       if( wmxssd_pcie_read_with_verify(test_buffer, i, (unsigned int *)ramdisk_writeaddr, knownvalue_buffer) )
//       {
//          failed = 1;
//       }
//       
